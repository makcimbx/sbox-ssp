...
namespace TLC.SSP
{
	...
	[Library( "ent_space_ship", Title = "Space Ship", Spawnable = true )]
	public partial class SpaceShipEntity : Prop, IUse
	{
		...
		private SpaceShipWheel frontLeft;
		private SpaceShipWheel frontRight;
		private SpaceShipWheel backLeft;
		private SpaceShipWheel backRight;
		...
		protected override void OnDestroy()
		{
			...
			if ( driver is GroundPlayer player )
			{
				RemoveDriver( player );
			}
		}
		...
		public override void Simulate( Client owner )
		{
			...
			using ( Prediction.Off() )
			{
				...
				{
					if ( owner.Pawn is GroundPlayer player && !player.IsUseDisabled() )
					{
						RemoveDriver( player );
		...
		private void RemoveDriver( GroundPlayer player )
		{
			driver = null;
			player.Vehicle = null;
		...
		{
			if ( user is GroundPlayer player && player.Vehicle == null && timeSinceDriverLeft > 1.0f )
			{
				player.Vehicle = this;
				player.VehicleController = new SpaceShipController();
				player.VehicleAnimator = new SpaceShipAnimator();
				player.VehicleCamera = new SpaceShipCamera();
				player.Parent = this;
		...
		public override void StartTouch( Entity other )
		{
			...
			if ( other is GroundPlayer player && player.Vehicle == null )
			{
		...
		protected override void OnPhysicsCollision( CollisionEventData eventData )
		{
			...
			if ( eventData.Entity is GroundPlayer player && player.Vehicle != null )
			{
				return;
			}
